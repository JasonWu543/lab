...
step() 的阶段划分（必须按顺序）：
  ...
  4. update    →  append token 到 output_ids，检查终止条件
                  （max_new_tokens 或 EOS）；已完成则调用 scheduler.finish()，
                  否则调用 append_slot 为下一步准备 KV slot
...
class Engine:
    ...
    def _prefill_one(self, seq: Sequence) -> int:
        ...
        # TODO（U3）：
        #   1. 用 _cached_kv_len(seq) 确定 cached_len；整个 prompt 命中时仍留
        #      最后一个 token 重算，避免给模型空 input_ids
